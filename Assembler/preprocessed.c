...
# 4 "main.cpp" 2
# 1 "/usr/include/c++/9.2.0/stdlib.h" 1 3
# 36 "/usr/include/c++/9.2.0/stdlib.h" 3
...
# 42 "/usr/include/c++/9.2.0/cstdlib" 2 3
...
# 37 "/usr/include/c++/9.2.0/stdlib.h" 2 3
...
# 7 "main.cpp" 2
# 16 "main.cpp"
int parseParams(int argc, char *argv[], FORMAT *format, char **filename);
...
char *
generateMachineCode(FILE *source, int lines, int *fileSize, int *labels = 
# 25 "main.cpp" 3 4
                                                                         __null
# 25 "main.cpp"
                                                                             , int *err = 
...
int main(int argc, char *argv[]) {
    FORMAT format = manul;
    char *filename = (char *) "prog.asm";

    if (parseParams(argc, argv, &format, &filename) == -1) return -1;

    printf("\x1b[34m" "Compiling file %s in format %s.\n" "\x1b[0m", filename,
           format == manul ? "MANUL" : "TABBY");

    FILE *source = {};

    if (!loadFile(&source, filename, "r")) {
        printf("\x1b[31m" "Error while loading the file. Check the filename and permissions. Terminating...\n" "\x1b[0m");
        return -1;
    }
...
# 46 "main.cpp"
    ...
    FILE *output = fopen("compiled.bin", "wb");
...
char *generateMachineCode(FILE *source, int lines, int *fileSize, int *labels, int *err, int *errline) {
    ...
    for (int i = 0; i < lines; i++) {
        if(fscanf(source, "%s", cmd) == 0) break;
# 88 "main.cpp"
...
# 93 "main.cpp"
                    ...
                    *((int *)(machine_code)) = arg;
                    ...
                    printf("\x1b[31m" "SOSI HUI DOLBAEB TY VSE PROEBAL" "\x1b[0m");
                }
            }
        }
...
printf("%s %s %d\n", cmd, "POP", strcmp(cmd, "POP")); if (strcmp(cmd, "POP") == 0) { *(machine_code++) = 2; len += sizeof(char); for(int i = 0; i < 1; i++) { len += sizeof(int); if (fscanf({},"%d", &arg) != 
# 2 "commands.h" 3 4
(-1)
# 2 "commands.h"
) { *((int *)(machine_code)) = arg; machine_code = (char *)((int *)machine_code + 1); } else { printf("\x1b[31m" "SOSI HUI DOLBAEB TY VSE PROEBAL" "\x1b[0m"); } } }
printf("%s %s %d\n", cmd, "ADD", strcmp(cmd, "ADD")); if (strcmp(cmd, "ADD") == 0) { *(machine_code++) = 3; len += sizeof(char); for(int i = 0; i < 0; i++) { len += sizeof(int); if (fscanf({},"%d", &arg) != 
# 3 "commands.h" 3 4
(-1)
# 3 "commands.h"
) { *((int *)(machine_code)) = arg; machine_code = (char *)((int *)machine_code + 1); } else { printf("\x1b[31m" "SOSI HUI DOLBAEB TY VSE PROEBAL" "\x1b[0m"); } } }
printf("%s %s %d\n", cmd, "SUB", strcmp(cmd, "SUB")); if (strcmp(cmd, "SUB") == 0) { *(machine_code++) = 4; len += sizeof(char); for(int i = 0; i < 0; i++) { len += sizeof(int); if (fscanf({},"%d", &arg) != 
# 4 "commands.h" 3 4
(-1)
# 4 "commands.h"
) { *((int *)(machine_code)) = arg; machine_code = (char *)((int *)machine_code + 1); } else { printf("\x1b[31m" "SOSI HUI DOLBAEB TY VSE PROEBAL" "\x1b[0m"); } } }
printf("%s %s %d\n", cmd, "MUL", strcmp(cmd, "MUL")); if (strcmp(cmd, "MUL") == 0) { *(machine_code++) = 5; len += sizeof(char); for(int i = 0; i < 0; i++) { len += sizeof(int); if (fscanf({},"%d", &arg) != 
# 5 "commands.h" 3 4
(-1)
# 5 "commands.h"
) { *((int *)(machine_code)) = arg; machine_code = (char *)((int *)machine_code + 1); } else { printf("\x1b[31m" "SOSI HUI DOLBAEB TY VSE PROEBAL" "\x1b[0m"); } } }
printf("%s %s %d\n", cmd, "DIV", strcmp(cmd, "DIV")); if (strcmp(cmd, "DIV") == 0) { *(machine_code++) = 6; len += sizeof(char); for(int i = 0; i < 0; i++) { len += sizeof(int); if (fscanf({},"%d", &arg) != 
# 6 "commands.h" 3 4
(-1)
# 6 "commands.h"
) { *((int *)(machine_code)) = arg; machine_code = (char *)((int *)machine_code + 1); } else { printf("\x1b[31m" "SOSI HUI DOLBAEB TY VSE PROEBAL" "\x1b[0m"); } } }
printf("%s %s %d\n", cmd, "END", strcmp(cmd, "END")); if (strcmp(cmd, "END") == 0) { *(machine_code++) = 7; len += sizeof(char); for(int i = 0; i < 0; i++) { len += sizeof(int); if (fscanf({},"%d", &arg) != 
# 7 "commands.h" 3 4
(-1)
# 7 "commands.h"
) { *((int *)(machine_code)) = arg; machine_code = (char *)((int *)machine_code + 1); } else { printf("\x1b[31m" "SOSI HUI DOLBAEB TY VSE PROEBAL" "\x1b[0m"); } } }
...
int parseParams(int argc, char *argv[], FORMAT *format, char **filename) {
    if (argc == 1) {
        printf("\x1b[33m" "Neither file nor compiler type specified. Using default parameters\n" "\x1b[0m");
    } else if (argc == 2) {
        char *param = argv[1];
        if (strcmp(param, "manul") == 0)
            *format = manul;
        else if (strcmp(param, "tabby") == 0)
            *format = tabby;
        else
            *filename = argv[0];
    } else if (argc == 3) {
        *filename = argv[1];
        if (strcmp(argv[2], "manul") == 0)
            *format = manul;
        else if (strcmp(argv[2], "tabby") == 0)
            *format = tabby;
        else {
            printf("\x1b[31m" "Unknown program format %s. Terminating...\n" "\x1b[0m", argv[2]);
            return -1;
        }

    } else {
        printf("\x1b[31m" "Invalid number of arguments. Terminating...\n" "\x1b[0m");
        return -1;
    }
...
# 144 "main.cpp"
   "f"
# 144 "main.cpp" 3 4
   , "main.cpp", 144, __extension__ __PRETTY_FUNCTION__))
# 144 "main.cpp"
            ;
...
# 145 "main.cpp"
   "loadpath"
# 145 "main.cpp" 3 4
   , "main.cpp", 145, __extension__ __PRETTY_FUNCTION__))
# 145 "main.cpp"
                   ;
...
# 146 "main.cpp"
   "*loadpath"
# 146 "main.cpp" 3 4
   , "main.cpp", 146, __extension__ __PRETTY_FUNCTION__))
# 146 "main.cpp"
                    ;
...
# 147 "main.cpp"
   "mode"
# 147 "main.cpp" 3 4
   , "main.cpp", 147, __extension__ __PRETTY_FUNCTION__))
# 147 "main.cpp"
               ;
...
# 148 "main.cpp"
   "*mode"
# 148 "main.cpp" 3 4
   , "main.cpp", 148, __extension__ __PRETTY_FUNCTION__))
# 148 "main.cpp"
                ;
...
# 155 "main.cpp"
   "f"
# 155 "main.cpp" 3 4
   , "main.cpp", 155, __extension__ __PRETTY_FUNCTION__))
# 155 "main.cpp"
            ;
...
# 165 "main.cpp"
   "txt"
# 165 "main.cpp" 3 4
   , "main.cpp", 165, __extension__ __PRETTY_FUNCTION__))
# 165 "main.cpp"
              ;
    const char *pointer = 
# 166 "main.cpp" 3 4
